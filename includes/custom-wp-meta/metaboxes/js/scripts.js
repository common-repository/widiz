jQuery(function($) {
	...
	$('.meta_box_repeatable_add').live('click', function() {
		...
		clone.find('input, textarea, select').attr('name', function(index, name) {
			return name.replace(/\[(\d+)]/gi, function(fullMatch, n) {
				var next_num = Number(n) + 1;
				return '['+next_num+']';
			});
		});

		clone.find('input, textarea, select').attr('id', function(index, id) {
			return id.replace(/\_(\d+)_/gi, function(fullMatch, n) {
				var next_num = Number(n) + 1;
				return '_'+next_num+'_';
			});
		});
