...
define( 'CUSTOM_METABOXES_DIR', plugin_dir_url(__DIR__) . '/metaboxes' );
...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	$defaultLabel = isset( $field['default'] ) ? $field['default'] : 'Select one item';
	$repeatable_fields = isset( $field['repeatable_fields'] ) ? $field['repeatable_fields'] : null;
	$repeatable_prefix = isset( $field['repeatable_prefix'] ) ? $field['repeatable_prefix'] : '';
	...
	switch( $type ) {
		...
		case 'select':
		case 'chosen':
			echo '<select name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '"' , $type == 'chosen' ? ' class="chosen"' : '' , isset( $multiple ) && $multiple == true ? ' multiple="multiple"' : '' , 'data-value="'.$meta.'">
					<option value="">' . $defaultLabel . '</option>'; // Select One
		...
		case 'repeatable':
			...
			foreach( $meta as $row ) {
				echo '<tr>
						<td></td><td><div class="repeatable-block">';
				foreach ( $repeatable_fields as $repeatable_field ) {
					...
					$repeatableClass = 'repeatable-item-';
					if($repeatable_prefix){
						$repeatableClass .= $repeatable_prefix;
						$repeatableClass .= '_';
					}
					$repeatableClass .= $repeatable_field['id'];

					echo '<div class="repeatable-item '.$repeatableClass.'">';
				...
				echo '</div></td><td><a class="meta_box_repeatable_remove" href="#"></a></td></tr>';
				$i++;
			} // end each row
			echo '</tbody>';
			echo '
